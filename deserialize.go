	...
	if structValue.Kind() == reflect.Struct && structValue.NumField() > 0 {
		...
		for i := 0; i < structValue.NumField(); i++ {
			...
			if field.Anonymous && field.Type == reflect.TypeOf(Model{}) {
				...
				modelFieldPtr := unsafe.Pointer(modelFieldValue.UnsafeAddr()) //nolint:gosec // G103: intentional use of unsafe for unexported field access
				originalCopyFieldType := field.Type.Field(0) // Model.originalCopy field
				originalCopyFieldPtr := unsafe.Pointer(uintptr(modelFieldPtr) + originalCopyFieldType.Offset) //nolint:gosec // G103: intentional use of unsafe for unexported field access
...
func buildFieldMapFromPtr(ptrValue, structValue reflect.Value) map[string]reflect.Value {
	...
	structAddr := unsafe.Pointer(ptrValue.Pointer()) //nolint:gosec // G103: intentional use of unsafe for reflection
			...
			if field.Anonymous {
				...
				if embeddedType.Kind() == reflect.Ptr {
					if fieldValue.IsNil() {
						...
						fieldPtr = unsafe.Pointer(fieldValue.Pointer()) //nolint:gosec // G103: intentional use of unsafe for reflection
					} else {
...
func convertIntToInt32(v int) (int32, error) {
	...
	return int32(v), nil //nolint:gosec // G115: integer overflow conversion - bounds checked above
}
