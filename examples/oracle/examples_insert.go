...
func Example7_InsertAndReturn(ctx context.Context, db *typedb.DB, firstUser *User) {
	...
	insertedPost, err := typedb.InsertAndReturn[*Post](ctx, db,
		"INSERT INTO posts (user_id, title, content, published) VALUES (:1, :2, :3, :4) RETURNING id, user_id, title, content, published, created_at",
		newPost.UserID, newPost.Title, newPost.Content, newPost.Published)
	if err != nil {
