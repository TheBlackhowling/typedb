...
func TestMSSQL_Logging_Exec(t *testing.T) {
	...
	t.Run("success logs debug", func(t *testing.T) {
		...
		_, err := db.Exec(ctx, "INSERT INTO users (name, email) VALUES (@p1, @p2)", "Test User", "test@example.com")
		if err != nil {
...
func TestMSSQL_Logging_PerInstanceLogger(t *testing.T) {
	...
	_, err = db.Exec(ctx, "INSERT INTO users (name, email) VALUES (@p1, @p2)", "Test User", "test2@example.com")
	if err != nil {
		t.Fatalf("Exec failed: %v", err)
	}
...
func (u *UserWithNolog) QueryByID() string {
	return "SELECT id, name, email, password FROM users WHERE id = @p1"
}
